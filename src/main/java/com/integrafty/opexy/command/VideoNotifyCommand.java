...
public class VideoNotifyCommand implements SlashCommand {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        StringBuilder sb = new StringBuilder("### Tracked YouTube Channels\n");
        if (list.isEmpty()) {
            sb.append("*No channels tracked yet.*");
        } else {
            for (NotificationEntity e : list) {
                sb.append(String.format("● **%s** - `%s`\n", e.getDisplayName(), e.getChannelId()));
            }
        }
        ...
        ActionRow row = ActionRow.of(
            Button.success("notify_add_video", "Add Channel"),
            Button.danger("notify_remove_video", "Remove Channel")
        );

        Container container = EmbedUtil.containerBranded("NOTIFICATIONS", "YouTube Video Manager", sb.toString(), EmbedUtil.BANNER_MAIN, row);
