...
@Component
public class SpeedChallengeCommand extends MultiSlashCommand {
    ...
    public SpeedChallengeCommand(EventManager eventManager, AchievementService achievementService) {
        super("speed", "بدء تحدي الـ 7 ثواني (Minecraft Edition)");
        ...
        event.replyEmbeds(embed.build()).queue(hook -> {
            event.getChannel().getIterableHistory().takeAsync(1).thenAccept(messages -> {
                ...
                event.getChannel().sendMessage("⏳ بدأ العد التنازلي...").queueAfter(7, TimeUnit.SECONDS, msg -> {
                    // Logic to remove listener if no one won
                });
            });
        });
    }
    ...
    public SlashCommandData getCommandData() {
        return Commands.slash(getName(), getDescription())
                .addOption(OptionType.STRING, "difficulty", "صعوبة التحدي", true)
