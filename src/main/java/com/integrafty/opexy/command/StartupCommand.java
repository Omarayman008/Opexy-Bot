...
public class StartupCommand implements SlashCommand {
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        Container mapContainer = EmbedUtil.containerBranded("MAP", "HighCore Explorer", mapContent, EmbedUtil.BANNER_MAIN);

        // 2. Roles & Socials Section
        String settingsContent = 
            "### ⚙️ الإعـدادات والـتـنـبـيـهات | SETTINGS\n" +
            "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n" +
        ...
        event.getChannel().sendMessage(new MessageCreateBuilder().setComponents(mapContainer).useComponentsV2(true).build()).useComponentsV2(true).queue();
        event.getChannel().sendMessage(new MessageCreateBuilder().setComponents(settingsContainer).useComponentsV2(true).build()).useComponentsV2(true).queue();
